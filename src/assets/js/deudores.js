// ============================================================
// DEUDORES - Firestore (colecciones "deudores" y "clientes")
// ============================================================
import {
    collection,
    onSnapshot,
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (formAdmin) {
        formAdmin.addEventListener("submit", async (evento) => {
            ...
            let btnGuardar = formAdmin.querySelector("button[type=submit]");
            ...
            try {
                await addDoc(coleccionClientes, {
                    nombre: nombre,
                    correo: correo,
                    detalle: mensaje,
                ...
                mensajeAlerta.textContent =
                    "¡Registro exitoso! Los datos de " + nombre + " han sido guardados en Firestore.";
                formAdmin.reset();
            } catch (error) {
                console.error(error);
                mensajeAlerta.style.color = "red";
                mensajeAlerta.textContent = "No se pudo guardar el registro. Intenta nuevamente.";
            } finally {
...
function escucharDeudores() {
    let contenedor = document.getElementById("contenedorDeudores");
    if (!contenedor) return;

    contenedor.innerHTML = `<p class="estadoCarga"><span class="spinner"></span>Cargando fiados desde Firestore...</p>`;

    onSnapshot(
        coleccionDeudores,
        (snapshot) => {
            listaDeudores = snapshot.docs.map((docItem) => ({
                id: docItem.id,
                ...docItem.data(),
            }));
            renderizarDeudores();
        ...
        (error) => {
            console.error(error);
            contenedor.innerHTML = `<p class="mensajeError">⚠️ No se pudieron cargar los fiados. Verifica tu conexión o permisos.</p>`;
        }
    );
}
...
function renderizarDeudores() {
    ...
    if (listaDeudores.length === 0) {
        ...
        h2.textContent = "No hay fiados pendientes";
        contenedor.appendChild(h2);
        return;
    }

    listaDeudores.forEach((deuda) => {
        ...
        btnPagar.addEventListener("click", () => cobrarFiado(deuda));
...
async function cobrarFiado(deuda) {
    let confirmacion = confirm(
        "¿Confirmas el pago de $" + deuda.totalDeuda + " por parte del cliente " + deuda.cliente + "?"
    );
    if (!confirmacion) return;

    try {
        // 1. Registrar la venta pagada en el historial (Firestore)
        await addDoc(coleccionVentas, {
            fecha: new Date().toLocaleString(),
            productosComprados: deuda.productos,
            totalCobrado: deuda.totalDeuda,
            metodoPago: "Fiado (Pagado por " + deuda.cliente + ")",
        });

        // 2. Eliminar el fiado ya cobrado (deleteDoc)
        await deleteDoc(doc(db, "deudores", deuda.id));
